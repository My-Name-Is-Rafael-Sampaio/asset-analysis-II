...
def get_user_choice():
    ...
    choice = input(
        Fore.YELLOW
        + "Digite o número da sua escolha (ou 'q' para sair): "
        + Style.RESET_ALL
    )
    return choice
    ...
    while True:
        ...
        if ticker:
            ...
            direction = market_direction(
                metrics["Last_SMA_50"], metrics["Last_SMA_200"]
            )

            print(
                f"\n"
                + Style.BRIGHT
                + f"Análise para {ticker} (Até {end_date}):"
                + Style.RESET_ALL
            )
            print(f"Último Preço de Fechamento: ${metrics['Last_Close']:.2f}")
            print(
                f"Última Média Móvel Simples de 50 dias: ${metrics['Last_SMA_50']:.2f}"
            )
            print(
                f"Última Média Móvel Simples de 200 dias: ${metrics['Last_SMA_200']:.2f}"
            )
            ...
            print(
                Fore.RED
                + "Escolha inválida. Por favor, digite um número válido ou 'q' para sair."
                + Style.RESET_ALL
            )
